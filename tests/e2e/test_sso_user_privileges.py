...
def test_sso_local_user_privilege_mapping_chromium(
    ...
    )  # pfsense_user fixture creates user with page-all privileges


@pytest.mark.usefixtures("saml2_config_default")
def test_sso_for_non_local_user_with_no_priv_chromium(
    params: Params, chromium_browser: Browser
) -> None:
    """
...
@pytest.mark.usefixtures("pfsense_user")
@pytest.mark.usefixtures("saml2_config_no_groups")
def test_sso_local_user_privilege_mapping_firefox(
...
@pytest.mark.usefixtures("saml2_config_default")
def test_sso_for_non_local_user_with_no_priv_firefox(
    params: Params, firefox_browser: Browser
) -> None:
    """
    Ensure SSO logins for a user that is not mapped to a local user or group are denied access.
